...
export const slide=(progress:number,from:Readonly<{x:number;y:number}>,to:Readonly<{x:number;y:number}>={x:0,y:0})=>({x:lerp(from.x,to.x,progress),y:lerp(from.y,to.y,progress)})
