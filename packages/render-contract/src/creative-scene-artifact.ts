...
export const validateCreativeSceneArtifactV1 = (value: unknown): CreativeArtifactResultV1<CreativeSceneArtifactV1> => {
  ...
  if (!isRecord(artifact.governance) || (artifact.governance.artifactPurpose !== undefined && artifact.governance.artifactPurpose !== 'production' && artifact.governance.artifactPurpose !== 'review') || artifact.governance.structuralQaPassed !== true || !bounded(artifact.governance.styleLockId, 240) || !bounded(artifact.governance.creativeLanguageId, 240) || typeof artifact.governance.cohesionScore !== 'number' || !Number.isFinite(artifact.governance.cohesionScore) || !Array.isArray(artifact.governance.requiredCapabilities) || artifact.governance.requiredCapabilities.some((item) => !bounded(item, 240)) || !isRecord(artifact.governance.reviewEvidence) || !bounded(artifact.governance.reviewEvidence.canonicalReviewRef, 2048)) return Object.freeze({ ok: false, refusal: Object.freeze({ code: 'CREATIVE_ARTIFACT_INVALID', message: 'Creative artifact governance/review evidence is invalid.' }) })
