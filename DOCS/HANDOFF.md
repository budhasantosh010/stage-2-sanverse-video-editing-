...
Updated 2026-08-01. **P1-F.0.1 - Studio Workspaces and Docking V1 is technically complete.** Studio exposes Edit, Effects, Color, and Audio as accessible Studio-only presentation views over one existing editor authority. One `EditProject`, revision, history, Undo/Redo stack, playhead, Timeline selection/viewport, Canvas/Inspector draft, native video, AI conversation, proposal, preview, and export path survive every switch.
