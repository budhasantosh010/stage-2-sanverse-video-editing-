...
Updated 2026-08-03. **P1-F.0.2 — Nested Studio Layout Engine V2 is complete.** Studio uses `react-resizable-panels@4.12.2` behind a typed/versioned presentation contract with V1 migration, safe fallback, persisted completed resizes, live nested preset geometry, non-destructive responsive overlays, and keyboard separators. It preserves one existing `EditProject`, revision, history, Undo/Redo stack, playhead, Timeline selection/viewport, Canvas/Inspector draft, native video, AI conversation, proposal, preview, and export path. Final suites pass 1,158/1,158 and the production build passes. Evidence is in `DOCS/evidence/2026-08-03-p1f02-nested-studio-layout-v2/`. P1-F.1 has not started.
