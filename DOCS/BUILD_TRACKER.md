...
| G3-02 | G3 | Owner-facing accepted-history export and download | In progress | Owner's first real Export reached a generic failure; managed replay identified blocked `ffprobe` launch (`spawn EPERM`). Safe 503 classification, actionable client message, and Retry now pass a direct adapter/client/API contract plus API build/web type-check. Original normal-PowerShell error was not persisted; owner E4 retry, timing, and pixel review remain open |
