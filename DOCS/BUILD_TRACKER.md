...
| MOTION-INGEST-V1 | Plan A engineering infrastructure | One fail-closed external-component inspect→ingest→productize→parity→register pipeline; CH1 Frosted Icon Rail first pilot | Engineering complete; owner integrated-parity gate open | All 10 CH1 source packages approval/determinism/direct-seek inspect PASS; pilot source hash locked; 14 semantic nodes / 15 exposures / 10 editable C5 tracks; C3/C4/C5 + 4 ratios + AI-editability PASS; 31-frame real-browser source↔integrated parity, full-video SSIM 0.996780 supporting evidence; 469/469 scoped tests + root build PASS; pre-owner register correctly refused; public catalog stays 89; `apps/web` unchanged; `DOCS/motion/evidence/COMPONENT-INGEST-V1.md` |
