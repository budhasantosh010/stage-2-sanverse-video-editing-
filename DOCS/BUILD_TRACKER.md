...
September 9: decoder foundation complete (13/13); opt-in v10 layer contract and FFmpeg composition implemented, not released in UI. Real four-second/120-frame overlap render passes. Rendering regression 269/269; API/contract/web builds pass. Live preview, retimed/dynamic motion fidelity, independent extracted audio and actual-user workflow remain OPEN. No guards removed; Motion untouched; no push.
...
Final current checkpoint: transfer plus retiming loop verified through actual revision-30 rendered MP4 and real half-speed preview. Follow-up visual-order guard automated checks pass: domain 568/568, planner 9/9, transfer/render 6/6 and all-workspace build. Its last browser walkthrough is blocked by webview attachment (INFRA-014), NOT by an application HTTP failure (2010 returns 200). FAIL-077 MONITORING; T5.5 remains OPEN for genuine layer compositing/independent audio and owner confidence.

Latest continuation (supersedes PARKED below): bounded primary transfer is active. Preview-owning-track and linked-audio drag routing fixed; 22/22 focused tests and all-workspace build pass. Real transfer/Undo/Redo/track-visibility browser checks pass at revisions 23–29, one video. Export validation and retiming-source regression are in progress. FAIL-069 remains OPEN for arbitrary overlapping layers/independent audio.

Primary transfer attempt: PARKED, not shipped. Candidate domain 567/567 and planner 7/7 passed; required preview test execution was denied by approval-service usage exhaustion. Preserved candidate text, restored earlier runtime source; no transfer completion tick, no build/browser claim for that candidate. FAIL-069 remains OPEN.

Latest closure: FAIL-073 export-feedback visibility RESOLVED; integration/App affected rerun 20/20, final web build PASS, real collapsed-AI → Export → visible Download MP4 passed at unchanged saved revision 22 with one video. Servers 2010/2011 and OpenCut 2012 restored. Supersedes the in-progress export wording below; remaining T5.5 checklist is unchanged.

Continuation: outside-drop cancellation, bounded edge scrolling, waveform name strip and relative gain dragging implemented. Final affected tests 77/77 plus production web build/ownership boundary PASS. Actual audio-body move at gain 0, Undo/Redo and revision-20 MP4/full decode pass. Live OpenCut comparison completed through split/layer transfer/audio extraction and movement; confirms Sanverse primary-track transfer remains missing. Export feedback visibility regression found during final walkthrough is being closed. T5.5 remains OPEN; no Motion integration or remote push.

Latest T5.5 checkpoint: linked drag/waveform/control-density repairs implemented; 135 focused tests passed after affected rerun; production build and protected-lane check PASS; real revision-7 MP4 exported and decoded. Confidence gate remains OPEN. Outstanding user-flow and tooling checks are explicitly listed in `DOCS/evidence/2026-09-06-t55-drag-reliability/T55_DRAG_RELIABILITY.md`; historical full-suite totals below are not new runs.
