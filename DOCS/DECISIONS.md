...
## DEC-023 — Creative Direction approval creates Style authority before Storyboard

- Status: Approved by the owner
- Date: 2026-09-01
- Decision: raw-video Creative Runs have four distinct authority locks in order: owner-approved Creative Direction creates the video-wide Approved Style Lock; owner-approved Storyboard locks exact scene design; owner-approved Animatic locks timing; owner-approved Motion locks animation. Brand Context is evidence only and never approval authority.
- A draft style recommendation has `creativeDirectionProposalId`/revision identity only. `styleLockId` is created only after exact trusted host approval and is derived from canonical approved recommendation + creative language + source/brand scope + proposal revision. Approval identity remains separate from style-content identity.
- Opportunity planning is downstream of Style authority. The V2 planner consumes `ApprovedStyleLockV1` and cannot create/revise it. Raw-video scene creation verifies the opportunity map was planned against the same Style Lock content hash/revision. Standalone low-level Storyboard constructors remain reusable outside this orchestration boundary.
- Hard approved style constraints never widen to fit a component. Semantic normalization maps component defaults into approved palette/font roles where deterministic; ambiguity/refusal is safer than silently accepting an off-brand default. Soft language remains QA guidance, and scene exceptions are explicit by scene/reason/dimension.
- Reopening Creative Direction invalidates active downstream derived authority but not accepted production state: opportunity map, active scene batch and downstream reviews become stale/cleared, while audit history may retain prior artifacts. No project revision/Undo entry is created until final approved scenes are atomically applied.
- Creative Direction review reuses the existing exact evidence/revision-bound chat-first review and host-only approval infrastructure. There is no model-callable approve tool and no second approval system.
- Why: the current raw-video planner auto-generates a `styleLockId` and Storyboards before the owner has seen or approved video-wide art direction, confusing recommendation with authority and allowing component defaults to widen style constraints. The new boundary makes style authority explicit without turning Style Lock into a scene template lock.
- Trade-off: raw-video creation gains one mandatory owner checkpoint and V1 run migration complexity. That extra round trip is intentional because it prevents an entire ten-scene video from being designed under an unreviewed global language.
- Revisit trigger: owner evidence shows the Creative Direction checkpoint is redundant for a narrowly scoped workflow, or future trusted reusable brand/video presets can themselves carry explicit owner-certified style authority without weakening exact revision/evidence binding.

## DEC-024 — Supplement component-native pixels with canonical authored graph pixels

- Status: Approved by the owner
- Date: 2026-09-01
- Decision: keep each registered component's bespoke React/CSS renderer for its shipped baseline nodes, and add one shared Motion Native Runtime supplement that traverses the exact evaluated canonical scene from `rootNodeId` and renders newly authored reachable visual nodes. Baseline component nodes are not generically redrawn.
- The supplement is part of `MotionComponentHost`, so Storyboard review, preview and export inherit the same behavior without a second renderer or MCP-only code path. It preserves ancestor group presentation and uses the existing `mergeMotionGraphNodeStyle` authority for exact-tick visibility, opacity, transform, blend, effects, masks and supplemental sibling stacking. The V1 supplement composites above the bespoke component surface; exact interleaving inside arbitrary component DOM subtrees remains outside this slice.
- Canonical shape/text/path/Expert nodes have direct native-browser surfaces. Image nodes require an already locally resolvable/browser-safe source; unresolved opaque sources refuse instead of becoming invisible pixels.
- Why: replacing all 99 bespoke component renderers with a generic graph renderer would lose layout and component-specific visual semantics that are not fully encoded as generic graph geometry, while leaving the old behavior drops valid newly authored graph nodes entirely. A supplement closes the missing-pixel hole without duplicating or regressing existing component-native visuals.
- Trade-off: same-ID/same-type baseline nodes still rely on their registered component renderer to honor graph-resolved presentation/content. The supplement guarantees pixel ownership for authored additions and type replacements; it does not pretend the generic graph schema contains every bespoke layout semantic of existing components.
- Revisit trigger: the canonical graph gains complete layout/geometry semantics for all baseline component pixels, allowing registered bespoke React renderers to be retired safely, or a baseline component repeatedly fails to honor same-node graph changes that cannot be corrected through its graph-aware renderer.
