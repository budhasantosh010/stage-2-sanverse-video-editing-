...
- The owner's first representative Export attempt reached the error state, so Task 8 is not outcome-complete. The original server detail was not persisted and cannot honestly be reconstructed. A controlled replay in the managed Codex environment returned HTTP 500 in 72 ms and captured the concrete failure at `ffprobe` launch: Windows denied the child process with `spawn EPERM`. Direct native FFmpeg still works on the same media, although the measured 44.5-second render took about 151 seconds.
- Export launch failures are now classified at both synchronous spawn and emitted-error boundaries. `EPERM`/`EACCES` becomes the allowlisted `RENDER_PROCESS_BLOCKED` code, the API returns a sanitized HTTP 503, and Studio explains that accepted edits remain safe, tells the owner to restart from normal PowerShell, and provides **Retry export**. A direct adapter/client/API contract, the API build, and the web type-check pass. Vite/Vitest and a fresh combined browser render remain blocked by this managed session's child-process policy, so the owner PowerShell re-test remains required.
