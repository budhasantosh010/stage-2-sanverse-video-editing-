...
- No backend, upload, persistence, AI integration, database, nameplate proposal/preview, renderer integration, or export has been implemented.
...
- Tasks 2 and 3 of the pointed-nameplate plan are implemented in isolation: the canonical contract fails closed and remains immutable, while Studio can capture one ephemeral point without permanently blocking native video controls. Nameplate composition, canonical history integration, preview, export, and every later task remain unimplemented
...
- Whether the explicit Point/Cancel/capture interaction is clear and fast on representative owner footage; automated tests cannot establish usability or perceived feel.
