...
### Parallel Motion Program checkpoint — MOTION-C3

The separate Motion worktree has completed the implementation and release-candidate verification of **MOTION-C3 Professional Layer Hierarchy** without changing `apps/web` or the production editor goal above. Layers are a pure projection of the same Motion Graph—there is no second Layer document. C3 separates constant node render-enable from animated visibility, keeps locks in persistent authoring metadata rather than pixels, computes ancestor-effective enable/lock state, and introduces one canonical selection shared by Layers, Preview, node Inspector, Effects, Masks and the existing C2 keyframe controls.

Motion Lab now has a development-only **Compositor** mode with graph-derived search/expand Layers, single/multi selection, Preview hit selection and bounds overlay, rename/reorder/local-transform reparent/duplicate/delete/group/ungroup, visibility, locks, effects/masks, high-value shortcuts and a bounded Lab-only Undo/Redo proof. All graph mutations continue through C1 operations. All **69 components × 4 ratios** project successfully; native Group/Text/Shape/Path/Image node types are covered. Real Edge proves Layer↔Preview synchronization, selection persistence, ancestor lock, effective hidden-parent behavior, grouping Undo/Redo, explicit DROP INSIDE reparent, and live `◆ fx M` badges. Cost Card composition pixels are byte-identical before/after grouping with overlays cleared. C3 release-candidate suites are **304/304** with **7/7 Motion workspace builds**. Evidence: `DOCS/motion/evidence/MOTION-C3.md`; architecture: `DOCS/motion/LAYER_ARCHITECTURE.md`. C3 is ready for its separate `motion-compositor-c3` Git checkpoint; **MOTION-A19 has not started yet and MOTION-C4 is not started.**
