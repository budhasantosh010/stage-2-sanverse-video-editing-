...
Remaining: agent-assisted sustained interaction/waveform/matched-comparison gate, then owner real-edit/audio/confidence acceptance. Those remain OPEN; do not claim parity or a completion percentage. Older paragraphs below describe their dated checkpoints, not the latest implementation state.
...
Live Studio now uses the shared v10 picture manifest for transferred primary footage, bounded muted decoders and one composition clock. Cross-track overlaps are accepted through existing typed history; same-track collisions, locks and revision guards remain. Reverse resources, linked-audio focus, drag reliability and gap playback repairs are included. Independent extracted dialogue is NOT implemented; FAIL-069 and owner confidence remain OPEN.

Latest evidence and remaining gates: `DOCS/evidence/2026-09-12-layered-timeline-release.md`. Earlier dated paragraphs below are historical, not current restrictions or test totals. Dedicated startup: `npm run dev:timeline` in the isolated timeline worktree; web 2010, API 2011. No reinstall required for the existing checkout.

Latest verification: 103/103 focused checks; production web build PASS, 310 modules, JS 1,008.05 kB / gzip 279.64 (+1.80 / +0.74 from the prior audio checkpoint), CSS unchanged 145.14 / gzip 24.42. Ownership boundary PASS, 82 paths, no Motion modifications/imports. Scoped diff check PASS; known font-runtime, chunk-size and line-ending warnings remain.

Latest checkpoint: reverse resources now Studio-wired; pending reverse seeks no longer jump to source zero; clicking selected linked dialogue correctly focuses Inspector (FAIL-088/089/090). Focused checks 103/103; actual reverse playback/Undo/Redo/exact seek and dirty-draft warning verified. Test content restored at saved change 58. Live layered canvas/clock, overlap release, independent extracted dialogue and final matched workflow remain OPEN; v9 and overlap guards remain. Active plan: DOCS/plans/2026-09-08-layered-editor-engine.md. No Motion changes or push. Server restarted hidden on 2010/2011, historical launcher PID 11740 (verify before stopping). Older checkpoint totals below are historical.

Final follow-up verification: 50/50 focused checks and web production build PASS (JS 1,006.25 kB / gzip 278.90; CSS unchanged). Ownership boundary PASS, 77 paths, no Motion modifications. Server launched as hidden node PID 7524 on 2010/2011 with logs in .sanverse-data/timeline-dev.{out,err}.log; PID is historical and must be verified before any later stop.
...
September 9 layered-engine checkpoint: decoder complete (13/13), opt-in v10 layer manifest/schedule and FFmpeg composition implemented. Rendering regression 269/269; real synthetic overlap render 1/1, full 120-frame decode. API/contract/web builds pass; JS 1,005.25 kB/gzip 278.43, CSS 145.14/gzip 24.42. Live compilation remains v9: NOT live layered-UI completion. Next is preview integration, retimed-motion parity, independent audio and actual-user workflow. FAIL-069 stays OPEN; guards stay; Motion untouched; no push. Dedicated server restarted on 2010/2011.

Owner approved the internal layered playback upgrade on September 8 ("yes approved"). Active plan: DOCS/plans/2026-09-08-layered-editor-engine.md. Replace the literal one-video-element restriction with one composition clock plus bounded synchronized decoders; preserve project/revision/proposal/history/export authority. Overlap guards remain until preview AND export support the same layered semantics. Motion remains untouched.
...
## Latest focused repair — read before earlier historical totals

Latest checkpoint, supersedes all in-progress statements below: bounded transfer and source-retiming fixes are verified locally, including real revision-30 1080p H.264/AAC export with complete decode and representative frames. FAIL-076 also has real half-speed UI proof. Follow-up FAIL-077 replaces blanket visual refusal with one shared track-order check and includes legacy nameplates: domain 568/568, drag planner 9/9, compiler/transfer 6/6, all-workspace build PASS. Final bundle JS 1,002.74 kB/gzip 277.60, CSS 145.14 kB/gzip 24.42. The last UI validation for this follow-up is BLOCKED: in-app browser webview will not attach (INFRA-014); server GET http://localhost:2010 returns 200. Next: attach browser, open saved test-30s.mp4, confirm outputs/speed, add title/B-roll above V2 footage, Undo/Redo/export. Preserve user changes and do not reset project. Arbitrary overlap/lower visual interleaving and independent extracted audio remain OPEN. No commit or push.

September 7 resumed: primary transfer is ACTIVE again in the isolated timeline worktree, superseding the parked checkpoint below. Preview owning-track routing (FAIL-074) and dialogue-to-video routing (FAIL-075) fixed with RED/GREEN evidence; focused preview/planner 22/22 and all-workspace build pass. Browser V1→V2 saved revision 23, Undo 24, Redo 25; visibility checks 26/27 passed and restored at 28/29. One video, linked sound retained on A1. Fresh revision-29 export is being verified. Speed/reverse source-resolver mismatch is the next reproduced hypothesis, under focused regression. Free overlapping layers and independent extracted audio remain incomplete; no parity claim or push. Use http://localhost:2010 (API 2011).

September 7 later attempt: primary-track-transfer candidate is PARKED, NOT ACTIVE. It passed 567 domain tests and 7 drag-planner tests, then exposed an unverified preview/output mismatch (primary-source.ts still reads V1 output instead of the owning track). The required preview regression command was rejected by automatic approval because that service hit its usage limit. No bypass attempted. Candidate source/tests were preserved in `DOCS/evidence/2026-09-06-t55-drag-reliability/PENDING_PRIMARY_TRANSFER.txt`; only that attempt's implementation/tests were removed from live source, restoring the pre-attempt behavior and preserving all earlier repairs. FAIL-069 remains OPEN. Resume at preview/output and shared compositing design, not startup or old drag fixes. See INFRA-012 and FAIL-074.

September 7 closure: servers restarted on 2010/2011 and competitor 2012; saved projects reused. Export-hidden-by-collapsed-AI defect is now RESOLVED (FAIL-073), superseding the in-progress wording below. Affected integration/App rerun 20/20 and final production build pass (JS 999.95 kB). Real Studio export revealed progress and ready/Download MP4 at the already-saved revision 22, with one video and no timeline mutation. OpenCut saved three-track project reopened. No full-suite rerun, commit, push, or parity claim; T5.5 remains OPEN.

Continuation checkpoint: edge autoscroll is now implemented (sustained edge-hold browser proof still open), outside releases cancel even if pointerup differs from the last move, and audio name/gain hit areas are separated with relative gain adjustment. Final five affected files passed 77/77; web build and ownership boundary passed. Real revision-18 audio-body move preserved 0 dB gain and one video, Undo/Redo passed at revisions 19/20. The resulting 34.466667s 1080p H.264/AAC MP4 fully decoded. Live OpenCut classic on 2012 now proves split, sideways movement, new-layer transfer, Undo/Redo, audio extraction and independent audio movement. Main-footage cross-track transfer remains the decisive Sanverse gap; no parity claim. A hidden export-result issue was found during this final walkthrough and is being closed with a focused continuity regression. Details and exact startup/recovery notes are in the Sep6 drag report.

T5.5 remains OPEN. Earlier candidate evidence below records the linked-drag/control-density repair and revision-7 export; use the continuation checkpoint above for the latest changes and test totals. Primary cross-track transfer, fresh independent-music browser proof, exact responsive proof, independent review and owner confidence acceptance are NOT complete. See `DOCS/evidence/2026-09-06-t55-drag-reliability/T55_DRAG_RELIABILITY.md`. Use the dedicated app at http://localhost:2010, not the Motion/MCP lane on 2000. No remote push.

Correction to older recording claims below: the 2.4x frame-difference statistic is descriptive only. Different actions, frame rates, durations, playback and crops confound it; it cannot establish latency, relayout causation, or comparative smoothness.
...
**Gates T0 through T5 are DONE. T5.5 Editor Confidence & Interaction Convergence is now the ACTIVE gate. T6 and T7 are NOT STARTED.** Final T5 gate: **2,501/2,501 tests**, all-workspace production build PASS, real Microsoft Edge 151 advanced-track workflow on real 30-second H.264/AAC media, reload/reopen and one-video continuity, responsive 1440/1024/390 proof, zero runtime/console errors, zero HTTP responses >=400, and a real revision-13 1280×720 H.264/AAC export whose downloaded and server-side SHA-256 match. T5 evidence is under `DOCS/evidence/2026-08-04-timeline-completion/T5_*.md` plus the adjacent `t5-*` machine-readable/screenshots/frame artifacts.

**T5.5 is a confidence gate, not a feature gate.** The dedicated Editor branch/worktree `timeline-t55-editor-confidence` is based exactly on verified T5 SHA `a89483deea214927bf44b6d82229ca6ffa72650c`. The current implementation fixes a real dead-enabled Razor route, adds safe tool Escape behavior, stronger primary/multi-selection and selected-track context, outside trim hit targets, drag/snap/tool feedback, internal Timeline vertical scrolling, creator-first control hierarchy, actionable empty states, and one shared Inspector numeric/draft language while preserving the existing Editor-owned Canvas. Machine verification is **2,510/2,510 tests PASS** and the all-workspace production build passes. Real Microsoft Edge 151 now also proves real Razor/select/Escape behavior, explicit Inspector dirty/reset state, sustained one-video playback, 1440/1280/1024/390 no-overflow continuity, zero browser/network failures, and a real revision-2 1280×720 H.264/AAC 30-second export whose downloaded/server SHA-256 both equal `27689ec436ed1a28d04a64fd22592851a4aca683dbe616d47becd62ff221da0a`. Evidence is under `DOCS/evidence/2026-08-11-editor-confidence/`. **T5.5 is not `[verified]` yet because its final 15–20 minute owner edit and same-task OpenCut confidence score are intentionally human-only acceptance gates. Per owner instruction, keep T5.5 in local Git only and do not push to GitHub until explicitly re-authorized after the CI/CD minute budget resets.**

**The owner's later confidence rejection reopened targeted T5.5 repair on 2026-09-03.** Live use found three no-op/dead-control paths that the prior scripted gate did not expose: the first pointer press on main footage could be lost when late filmstrip content replaced the target; ordinary Delete misclassified one linked footage/dialogue choice as a deliberate multi-selection; and Move Earlier/Later stayed enabled at sequence boundaries. These routes are repaired through the existing selection and edit authorities, with no new project model or edit engine. Focused Timeline verification is **22/22 PASS**, the web production build passes, and a fresh real-browser walkthrough on the active branch proved first-press selection, split, both valid reorder directions, delete-with-gap, Undo and Redo on isolated real media. This is focused repair evidence, not a replacement claim for the previously recorded 2,510-test complete regression. T5.5 remains open until the owner completes the unscripted same-task Sanverse/OpenCut comparison.

**The next bounded T5.5 slice removes three further confidence gaps without widening product scope.** Gapless primary footage is now directly draggable to a new sequence position, and the gesture compiles through the existing `reorder-clip` authority as one Undoable change rather than introducing free-position timing or a second edit engine. Timeline Zoom is closed behind one named disclosure by default, and the selected-item actions now sit immediately below the toolbar instead of below every track. Focused related verification is **100/100 PASS**, the all-workspace production build passes, and a real pointer walkthrough reordered two clips, saved change 12, then visibly Undo/Redo restored and reapplied the exact order as changes 13/14. Motion Graphics, T6/T7, the project model, renderer, preview and export paths remain untouched. T5.5 is still open for owner same-task OpenCut acceptance.

**Frame-by-frame comparison of the owner's 59.7-second OpenCut and 47.4-second Sanverse recordings now establishes a scope freeze: no more Timeline capabilities before confidence acceptance.** The complete streams were decoded and 1,421/1,183 Timeline-region frames analysed. Sanverse's recorded Timeline crop had about 2.4× OpenCut's mean frame-to-frame change and 56% more high-change frames per second, directionally confirming excess re-layout and permanent control density rather than a missing-feature problem. Precision playback, track creation and specialist controls now share one closed advanced disclosure; the four placement policies now use one compact selector. Focused verification is **37/37 PASS** and the all-workspace production build passes. Evidence is in `DOCS/evidence/2026-09-04-t55-opencut-recording-comparison/`.
