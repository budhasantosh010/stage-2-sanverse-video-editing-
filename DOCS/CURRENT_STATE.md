...
**Plan-A Component Ingest V1 is now implemented in an isolated local worktree and is stopped at the first-pilot owner parity gate.** All ten CH1 external source components are explicitly owner-approved and inspect as deterministic, direct-seek-safe `foreign → lossless-normalization` candidates. Only Component 01 Frosted Icon Rail advanced through intake/productization: immutable approved-source hash `428e6e32…`; 14 semantic graph nodes; 15 Creator/Designer/Advanced exposures; 10 editable C5 curve tracks; all four canonical ratios; C3/C4/C5 and typed AI-edit mappings pass. A real browser page runs the immutable approved CH1 snapshot beside the Sanverse-integrated renderer on one exact playhead. Full 31-frame engineering comparison preserves the same motion/composition; SSIM 0.996780 is supporting evidence only. Fresh scoped regression is **469/469 tests** and the root all-workspace build passes. A pre-owner `motion:register` call correctly refused, so the public catalog remains **89**. Next action is owner visual verification of the integrated pilot; only after that may registration and remaining CH1 ingestion proceed. Architecture: `DOCS/motion/COMPONENT_INGEST_ARCHITECTURE.md`; evidence: `DOCS/motion/evidence/COMPONENT-INGEST-V1.md`. `apps/web` remains untouched and A22/B2/B3/C6 remain unstarted.
