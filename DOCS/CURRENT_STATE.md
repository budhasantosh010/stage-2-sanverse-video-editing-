...
### Sanverse Creative Engine ABC-1 — Plan B0 Creative Direction

ABC-1 has begun in the same isolated Motion worktree under `DOCS/creative-engine/CREATIVE_ENGINE_MASTER_PLAN.md`: Plan A is Creative Capability, Plan B Creative Intelligence and Plan C Creative Control. **B0 Creative Direction Foundation is complete** and preserved by its dedicated `creative-direction-b0` checkpoint before C4 begins. New `@sanverse/creative-direction` uses the canonical 1,440,000-tick project clock and provides all eight semantic tracks, typed style/graphic/motion/footage/transition/emphasis/note/constraint directives, comments, creative-plan versions, typed serializable proposals, a vendor-neutral planner boundary, deterministic offline fixture planner and fail-closed validation. The development-only Creative Direction Lab supports exact-tick add/select/move/resize/delete/duplicate/type/property editing and shows typed fixture proposal resolution. Fresh combined Creative + Motion suites are **346/346 tests** with **8/8 workspace builds**. Real Edge inspected the Lab on strict port 2010. Evidence: `DOCS/creative-engine/evidence/CREATIVE-B0.md`. `apps/web` and the production editor remain untouched. **C4/A20/B1/C5/A21 are not started in this B0 checkpoint.**
