...
## Canonical Motion Graph Supplemental Renderer V1 — implementation + real-browser acceptance complete

REQ-026 / DEC-024 closes the renderer gap that allowed structurally valid authored Motion Graph nodes to disappear from review pixels. `MotionComponentHost` now keeps each registered component's bespoke React/CSS renderer for shipped baseline nodes and adds one shared supplemental canonical graph surface for newly authored reachable nodes. The supplement traverses the exact evaluated scene from `rootNodeId`, preserves authored ancestor visibility/opacity/transform/effects/masks/blend plus supplemental sibling order, and renders canonical shape/text/path/Expert nodes directly. Browser-safe/local image sources are permitted; unresolved opaque image sources fail closed instead of silently disappearing. The V1 supplement composites above the registered component surface and does not claim arbitrary interleaving inside bespoke component DOM subtrees.

The reported failure was reproduced red first: an enabled authored ellipse was present under `root.childIds`, passed graph validation, yet `MotionComponentHost` emitted no pixels for it. After the fix, Motion Native Runtime is **14/14 PASS**, including nested group presentation, disabled/unreachable omission, no baseline duplication and fail-closed image resolution.

Real Microsoft Edge then exercised the actual source-composited `/render/creative-scene` path using registered `sanverse.floating-value-cloud` pixels plus three authored canonical nodes (magenta ellipse, cyan rounded rectangle and custom text). All authored nodes were present and visibly styled; preview and export PNG SHA-256 were both `6ab09670b5645636303e0ec069073c0c41491e361c34cfce1e8aab833bf38c74`, proving byte-identical preview/export pixels at the same exact tick. The accepted production project revision remained `1`, so review-artifact staging/rendering caused **zero production mutation**. Durable non-media evidence and failure notes are under `DOCS/evidence/2026-09-01-motion-graph-renderer-v1/`.

The existing 73-tool Sanverse STDIO Storyboard-authoring audit also remains green after the renderer change and still leaves accepted production unchanged. The authoritative Windows single-fork all-workspace regression exits `0`; key counts remain Web **1,239/1,239**, API **411/411**, Creative Direction **46/46**, Creative Production Adapter **37/37**, Edit Domain **491/491**, Motion Library **202/202**, Motion Graph **148/148**, Render Contract **121/121**, Motion MCP **30/30**, and the changed Motion Native Runtime **14/14**. The root all-workspace production build exits `0`; only the existing runtime nameplate-font resolution and Vite chunk-size warnings remain. Production dependency audit is **0 vulnerabilities** across 153 production dependencies. This renderer fix does **not** convert broad engineering authorization into Storyboard/Creative Direction/Motion owner approval, and it does not create the REQ-020 final raw-video release tag.

## Pre-Storyboard Creative Direction + Approved Style Lock V1 — engineering implementation complete

Raw-video Creative Runs now have an explicit video-wide authority gate between Source Understanding and opportunity/Storyboard planning. `BrandContextV1` is bounded evidence only; `CreativeDirectionProposalV1` is persisted/revisable draft review state; only an exact trusted-host approval of one proposal revision/evidence hash can compile a content-derived `ApprovedStyleLockV1`. Opportunity planning and raw-video scene creation consume that exact lock and verify `styleLockId` + proposal revision + content hash provenance. Reopening direction invalidates active opportunity/Storyboard/Animatic/Motion authority without mutating accepted production state or creating Undo history.

The external registry is now **73 tools**, adding `creative.propose_direction`, `creative.get_direction`, `creative.revise_direction` and `creative.reopen_direction` while reusing the existing host-only generic review decision path. Creative Run persistence is V2; V1 auto-generated styles migrate only as awaiting-owner draft direction, with prior downstream material archived for audit rather than promoted as active authority. Workspace `.md` and bounded brand/brief/style/guideline `.txt` files may be classified as confined `brief` evidence; transcript intake refuses brief-classified files.

Approved hard palette/type constraints are immutable: component defaults are normalized into approved semantic roles where deterministic or refused; they never widen the Style Lock. Exact Style Lock content hash + Creative Direction revision now survive Storyboard → Motion → immutable Creative artifact and are mandatory in Render Contract validation.

Deterministic real-project STDIO acceptance is green with **73 tools**: planning is refused before approval, direction is proposed and revised to revision 2, stale revision-1 review approval is refused, exact revision-2 trusted test-host approval produces `stylelock_46622b0a92ffbfac`, and the opportunity map preserves its content hash before the existing `$29→£29` / ellipse / structural Storyboard authoring battery completes with source-composited review and zero production mutation. A separate three-process continuation audit proves the pending Creative Direction review survives process A→B, trusted approval occurs after reconnect, and the exact Approved Style Lock survives process B→C without production mutation.

Real Codex `0.144.1` independently proved the model-facing gate without fabricating human approval. Thread `01a05db2-1fcb-7222-a5eb-444f67385c1b` created `run_00d140v0`, received the required `CREATIVE_DIRECTION_APPROVAL_REQUIRED` refusal before and after revision, produced pending revision-2 review `review_01j0yrfk` with one Creative Direction board artifact, and confirmed production revision stayed `0`. Final marker: `SANVERSE_CODEX_DIRECTION_GATE_OK run_id=run_00d140v0 proposal_id=direction_00nwsubu revision=2 review_id=review_01j0yrfk artifact_count=1 plan_blocked=true production_revision=0`. Codex deliberately did **not** call the owner-decision tool; broad engineering authorization is not misrepresented as visual approval of this exact board.

Production dependency audit is **0 vulnerabilities at every severity** across 153 production dependencies. Final all-workspace regression/build/hygiene evidence for this slice is recorded under `DOCS/evidence/2026-09-01-creative-direction-style-lock-v1/`. The separate REQ-020 meaningful-spoken-video / real scene approvals / 1× final-watch release gate remains unchanged and no raw-video release tag is created by this engineering slice.

## General Storyboard Authoring Surface V1 — implementation + client acceptance complete

External coding agents now receive the typed expressive authority of the canonical Motion Graph inside the existing revision-fenced Storyboard sandbox instead of being limited to text/font-size/opacity template filling. Canonical graph coverage now includes static node properties, replace-node/subtree and semantic-part authoring; Storyboard supports atomic multi-KVS graph/design transactions, bounded authoring budgets, hard/soft style QA, fail-closed content binding, typed inspection/schema, presentation/KVS controls, lock/reopen and durable persistence. Exact Storyboard approval remains the design lock, and Motion Forge now compiles from the exact approved Storyboard structural baseline rather than silently returning to the original component candidate.

The external registry is now **69 tools**. Real persisted-project STDIO acceptance authored `$29 → £29`, changed a canonical shape to `ellipse`, performed a structural duplicate + semantic-part redesign, produced exact source-composited KVS review pixels/images with compact semantic diffs, and left accepted production revision `0` unchanged. Source-visible review uses the canonical media-analysis frame endpoint at its existing bounded **640 px** derived-preview width, then scales that exact-tick frame inside the full review compositor; the media decoder safety envelope was not widened.

A real Codex `0.144.1` acceptance is green: after typed fail-closed correction of model-generated arguments, Codex applied two revision-fenced canonical authoring transactions, re-inspected sandbox revision `3`, explicitly verified Unicode code point `163` (`U+00A3`) immediately before `29` plus `ellipse` across every relevant KVS state, received **2 source-composite review images** in chat, and confirmed production revision remained unchanged. Final marker: `SANVERSE_CODEX_AUTHORING_OK run_id=run_01ehbqzl scene_id=creative_scene_01srm0io review_id=review_00nry3ss image_count=2`.

The authoritative Windows single-fork root test command exits `0`; key exact counts from this run include Web **1,239/1,239**, API **411/411**, Edit Domain **491/491**, Motion Library **202/202**, Motion Graph **148/148**, Render Contract **120/120**, Creative Production Adapter **35/35**, Motion MCP **30/30**, Motion Agent Tools **30/30**, and Motion Storyboard **16/16**. The root all-workspace production build also exits `0`. Final hygiene passes `git diff --check`, with 0 `sites/**` changes, 0 raw-media additions, 0 private-path additions and 0 secret-like added lines; production dependency audit reports 0 vulnerabilities at every severity. Durable evidence and the transparent failure registry are under `DOCS/evidence/2026-09-01-general-storyboard-authoring-v1/`.

The separate REQ-020 human raw-video release gate is unchanged. This authoring slice does not create the immutable `sanverse-external-mcp-raw-video-v1` tag or claim an unseen 1× human watch/creative approval.

A post-RC release-readiness audit checked whether the remaining ten-scene benchmark could be completed autonomously from currently available local data. The persisted `check .mp4` source is a real ~29.49-second 1920×1080/30fps MP4 with audio, but its attached analysis transcript is synthetic one-cue text spanning the full clip; it is not proof of the actual spoken words or their timing. The shipped `NullTranscriptionAdapter` explicitly fails closed with `TRANSCRIPTION_DISABLED`, and no local/offline recognizer is wired. Therefore the current source cannot truthfully be split into ten real spoken source moments. All available autonomous engineering is complete; the next legitimate release step requires a meaningful spoken source plus truthful timed sidecar/transcription, followed by the real owner Storyboard/timing/Motion approvals and 1× final watch defined by REQ-020. Evidence: `DOCS/evidence/2026-09-01-raw-video-release-readiness/STATUS.md`.
...
Provider-independent continuation evidence now launches **three** independent Sanverse STDIO child processes against Creative Run V2, with each process fully closed before the next starts and each exposing **73 tools**. Process A creates pending Creative Direction review `review_01aggmt7`; process B restores the exact review/evidence/artifact identity and performs one trusted approval; process C restores the exact resulting `stylelock_6b04261d86f627f8` / content hash at `opportunity-planning`. All three observe production revision `0`, and all three report no handshake-timeout evidence. Exact timings and identities are in `DOCS/evidence/2026-09-01-mcp-continuation-v1/stdio-reconnect.json`.

The older real Codex same-thread process-resume proof remains valid historical client evidence: process A created thread `01a05984-09c9-77a0-a660-f22c44d334d1`, exited, and a separate `codex exec resume` process restored that exact thread and Sanverse state. The new Creative Direction Codex acceptance separately proves the current V2 model-facing gate without claiming visual owner approval. Durable continuation evidence remains under `DOCS/evidence/2026-09-01-mcp-continuation-v1/`.
...
The first true cold-start test exposed a Windows launch defect: invoking npm through `cmd.exe` split the installed `C:\Program Files\...` path. The STDIO runtime launcher now resolves npm's real `npm-cli.js` and invokes it with `node.exe`; API/web output is redirected to ignored `.sanverse-data/mcp/runtime` logs so STDIO stdout remains protocol-only. Direct SDK verification from a cold state reports **52 tools**, API ready, web ready, no legacy user token environment variable, and zero-project readiness.

The one-time installer has been applied to the actual installed clients. Codex `0.144.1` is persisted as STDIO and a fresh real Codex process, starting with API/web off, successfully called `production.list_projects`; Sanverse recorded `codex-mcp-client` / `0.144.1` with `ok=true`. OpenCode `1.18.8` is persisted as a local STDIO MCP, cold startup was observed launching its Sanverse STDIO/API/web processes automatically, and a real OpenCode model-driven call through the owner's existing 4097 server successfully invoked `production.list_projects`; Sanverse recorded `opencode` / `1.18.8` with `ok=true`. Claude Code `2.1.168` reports the user-scope STDIO `sanverse` entry as **Connected**; per the owner's instruction, configuration-level connectivity is sufficient for this slice and its separate model-session timeout is not pursued.

Focused regression is green: Motion MCP **27/27** and Creative Production Adapter **30/30**. The full documented Windows single-fork all-workspace matrix passes with exit 0 and retains the unchanged **2,990-test** inventory; root all-workspace production build also exits 0. Durable evidence: `DOCS/evidence/2026-08-30-zero-setup-local-mcp-v1/`.
...
Creative Engine V1.6 integrates the existing Creative Engine into the production `apps/web` editor without creating a second project, history, clock, Motion Graph, preview, export, approval or MCP authority. Production source context is revision-bound and exact-tick; Storyboard/KVS, Animatic, Motion Forge, Motion Review and C3/C4/C5/C6 all operate over the existing Creative Engine workflow and canonical `MotionSceneV1`. One semantic node selection is shared across the four deep projections. A manual C5 preset edit uses canonical Motion Graph operations and deliberately invalidates prior approvals before the changed graph can be reviewed/applied again.

The production Library surface exposes all **99** canonical entries truthfully. V1.6 has one native production adapter for **Kinetic Headline**; the other **98** remain explicitly `creative-preview-only` because the current production renderer has no lossless arbitrary-Motion-Graph serialization path. The supported adapter maps an owner-approved canonical Kinetic Headline candidate into one ordinary production change set containing `add-title` + `set-visual-properties`, carrying Creative lineage/provenance through the existing server-authoritative edit-domain path. Exactly one Undo removes the complete accepted result, one Redo restores it, and the accepted project survives reopen. MCP uses a production-fenced view of the same internal registry/workflow, requires the live project revision for mutations and exposes no isolated apply/Undo authority.

Fresh real Microsoft Edge production evidence uses a temporary synthetic-real **640×360, 6.000 s** MP4 and proves: source load; Creative workspace; manual C5 `snappy` edit; stale-approval discard; Storyboard/Animatic/Motion reapproval; one atomic production apply at revision **1**; Undo at **2**; Redo at **3**; persisted reopen; desktop **1440×900**, tablet **1024×768** and mobile **390×844** with no horizontal overflow and exactly one video; globally reachable export completion even when the AI panel is collapsed; and a real H.264/AAC **640×360, 6.000 s** MP4 export of **858,823 bytes**, SHA-256 `653afd857d0fc4d3441c9486dfac564af9d18e4503c80f45c6f1b96287ce12ef`. Browser evidence has **0 console errors, 0 network failures and 0 bad responses**. Retained proof: `DOCS/evidence/2026-08-28-creative-engine-v16-production/`.

Final authoritative Windows single-fork repository matrix is **2,950 / 2,950 PASS across 25 workspaces**, including production `apps/web` **1,238 / 1,238**, Creative production adapter **12 / 12**, API **403 / 403**, edit-domain **488 / 488**, Motion Graph **139 / 139**, Motion Library **202 / 202**, Motion MCP **18 / 18** and render-contract **119 / 119**. Root all-workspace production build passes. Final hygiene is green: `git diff --check` PASS, `sites/**` diff **0**, raw media/export additions **0**, secret-like additions **0**, and the parallel-authority scan has no real match (its sole lexical hit is a canonical `1_440_000`-timescale test fixture). The coherent local release is sealed by immutable tag `sanverse-production-editor-creative-engine-v1.6`; no GitHub remote operation is part of this release. Durable status: `DOCS/motion/creative-engine-v1.5-v1.6/STATUS.md`.
...
The owner-approved continuous V1.2→V1.4 program is complete in the isolated Motion worktree. Corrective V1.2.1 remains preserved at immutable local tag `sanverse-creative-engine-v1.2.1`; V1.3 remains sealed at implementation/evidence commit `6821ff0930beff8e260f172ab4b24011334f4d49` and immutable local tag `sanverse-creative-engine-v1.3`; V1.4 is sealed at implementation/evidence commit `b6a833f3f8254c0a2cf5c1b4b54ebc9cb12ffaf7` and immutable local tag `sanverse-creative-engine-v1.4`, verified to resolve to that exact release commit. V1.4 adds one bounded canonical Motion Graph `expert` leaf plus `@sanverse/motion-expert-runtime`; there is still one graph, one exact 1,440,000-tick clock and no ExpertGraph/ParticleGraph/ShaderTimeline.

V1.4's fixed expert programs are procedural `orbital-rings`, analytically reconstructable `radial-burst` particles and bounded `plasma-field` shader plans. Every output is recomputed from exact tick + typed bounded parameters/assets + explicit seed, so direct/backward/random seek at the same tick is identical. Unknown/over-budget programs, arbitrary JavaScript/GLSL/WGSL, URLs, callbacks, filesystem/network access and autonomous timing fail closed. The existing rights/provenance-gated external bridge materializes only bounded declarative procedural/shader subsets; three vetted recipes and internal expert tools land in the existing registry first, while MCP remains a thin generic adapter with no expert/project/approval/Undo state.

Fresh real Edge `/expert-motion-review` ran true **1×** for **5075 ms**, captured **141** frames and reached exactly **7,200,000/7,200,000** ticks with direct/back/random seek PASS, resource QA PASS, C3/C6 expert IDs 3/3, three expert surfaces, 8 rings, 72 analytic particles, shader seed 303, zero release console errors and zero network failures. Retained entrance/middle/final frames were manually inspected and visually clean. Focused gates are Motion Graph **139/139**, Expert Runtime **9/9**, External Bridge **18/18**, Agent Tools **25/25**, MCP **16/16**, Native Runtime **8/8**, Motion Lab **66/66**. The final pre-release matrix passed **2,909/2,909**, and the post-tag repository-authoritative Windows single-fork matrix also passed **2,909/2,909**, including protected `apps/web` **1,231/1,231**; both root all-workspace builds passed. V1.3→V1.4 isolation audit shows `apps/web/**` diff 0 and `sites/**` diff 0; raw user/source media is absent; no GitHub remote/Actions work occurred; C13, universal Three/WebGL, AEP/MOGRT and production-editor integration were not started.

### Sealed V1.3 baseline
...
The existing external-source bridge now treats Rive truthfully. Only a bounded deterministic `sanverse.rive-subset/v1` export can materialize into native `MotionSceneV1` graph/keyframe state; raw `.riv` binaries and state machines refuse as `RIVE_RUNTIME_REQUIRED` instead of becoming a second runtime authority. Internal V1.3 operations land in `@sanverse/motion-agent-tools`; MCP remains the same thin generic adapter with no camera/preference/project/approval/Undo state.

Fresh real Edge `/camera-depth-review` ran true **1×** for **5021 ms**, captured **155** frames, reached exactly **7,200,000/7,200,000** ticks, proved C9 composed = true, graph-backed C10 = true, four depth layers with four distinct transforms, B8 `restrained`, Rive `native-materialize`, zero release console errors and zero network failures. Retained entrance/middle/late frames were manually inspected and visibly show the foreground responding most strongly while the tracked callout continues independently. Final deterministic repository matrix: **2,883/2,883 PASS**, including protected `apps/web` **1,231/1,231**; root `npm run build` **PASS / exit 0**. `apps/web/**` actual content diff is 0, raw source media is absent, the separately versioned `sites/` repository is explicitly root-ignored/excluded, and no GitHub remote/Actions work occurred. Durable program status: `DOCS/motion/creative-engine-v1.2-v1.4/STATUS.md`.
...
**Plan-A Component Ingest V1 CH1 ingestion is complete locally.** The owner directly approved the synchronized Sanverse integration for Component 01 Frosted Icon Rail, then explicitly authorized this Sanverse coding agent to preserve, engineering-verify and insert the remaining already-approved CH1 source components. Components 01–10 are now independently productized as canonical Motion Graph modules with typed props/exposures, exact source durations, deterministic/direct-seek behavior, all four canonical ratios, reduced motion, C3 Layers, C4 Timeline, C5 curves and typed AI-edit mappings. Components 02–10 each passed seven exact-tick approved-source↔integrated checkpoints; material normalization/evidence failures were fixed before registration (`MOTION-FAIL-022` through `MOTION-FAIL-026`). All ten are registered and appear in the real **Owner Approved CH1** Creative Library collection with fresh posters and `A · PASSED` reviews. Public catalog is now **99**; 10/10 new entries completed real Edge canonical 1× playback; durable review store is 99/99. Fresh scoped regression is **473/473 tests** and the root all-workspace build passes. `apps/web` remains untouched, no source/reference video bytes were copied, no new external animation runtime was added, local Git only, and A22/B2/B3/C6 remain unstarted. Architecture: `DOCS/motion/COMPONENT_INGEST_ARCHITECTURE.md`; evidence: `DOCS/motion/evidence/COMPONENT-INGEST-V1.md`.
