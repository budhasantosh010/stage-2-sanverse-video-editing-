...
- REQ-026 Canonical Motion Graph Supplemental Renderer V1 is engineering/browser complete: registered component-native baseline pixels remain bespoke, while newly authored reachable canonical graph visuals receive one shared runtime pixel path used by Storyboard review/preview/export. Real source-composited Edge proof shows native Floating Value Cloud pixels plus authored shapes/text together with byte-identical preview/export screenshots and no production revision mutation. The V1 supplement composites above bespoke component pixels; arbitrary interleaving inside component DOM subtrees is not claimed.
