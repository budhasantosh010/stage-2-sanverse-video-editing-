...
**Active implementation line:** `external-mcp-raw-video-v1`; the latest isolated engineering slice is REQ-026 / Canonical Motion Graph Supplemental Renderer V1. Verify current HEAD/remote SHA with Git before resuming because this handoff is updated before the final commit identifier is inserted.

**Current completed engineering slice:** the shared native renderer no longer silently drops newly authored canonical Motion Graph visuals. Registered component-native React/CSS remains responsible for shipped baseline pixels; `MotionComponentHost` now supplements that surface with newly authored reachable shape/text/path/Expert nodes from the exact evaluated canonical scene, preserving authored ancestor presentation and supplemental ordering. Unresolvable opaque image sources fail closed. Real source-composited Edge evidence shows registered Floating Value Cloud pixels plus authored magenta/cyan shapes and custom text together, with byte-identical preview/export PNG SHA-256 `6ab09670b5645636303e0ec069073c0c41491e361c34cfce1e8aab833bf38c74` and unchanged production revision. V1 supplemental nodes composite above bespoke component pixels; arbitrary interleaving inside bespoke component DOM subtrees is explicitly not claimed.

The prior REQ-025 Creative Direction gate remains authoritative: raw-video Creative Runs require video-wide Creative Direction authority after Source Understanding and before opportunity planning/Storyboard. The standard local Sanverse MCP exposes **73 tools**, including `creative.propose_direction`, `creative.get_direction`, `creative.revise_direction` and `creative.reopen_direction`. Brand Context remains evidence; draft direction remains revisable review state; only exact trusted-host approval of one review revision/evidence hash creates a content-derived Approved Style Lock. That exact Style Lock hash/revision is carried through opportunity planning, Storyboard/Motion and immutable Creative artifacts. V1 runs migrate without fabricated approval, and reopening direction invalidates downstream active authority without production mutation.
...
**Machine-verifiable gate:** green. The authoritative Windows single-fork root test command exits 0 with Web 1,239/1,239, API 411/411, Creative Direction 46/46, Creative Adapter 37/37, Edit Domain 491/491, Motion Library 202/202, Motion Graph 148/148, Render Contract 121/121, Motion MCP 30/30, Agent Tools 30/30, Storyboard 16/16 and the changed Motion Native Runtime 14/14. The all-workspace production build exits 0, production dependency audit is zero vulnerabilities at every severity, and the dedicated source-composited Edge renderer audit has exact preview/export SHA equality with zero production mutation. Verify the final Git SHA parity from the newest evidence/HEAD when resuming.
...
runbook, `DOCS/REQUIREMENTS.md` REQ-020/023/024/025/026, `DOCS/DECISIONS.md` DEC-021/022/023/024,
