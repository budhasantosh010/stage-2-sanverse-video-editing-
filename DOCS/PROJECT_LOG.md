...
- Upgraded Creative Run persistence to V2 with safe V1 migration. Legacy auto-generated style/downstream material is retained for audit but is not migrated as active approval authority. Reopening Creative Direction invalidates active opportunity/Storyboard/Animatic/Motion lineage without mutating production or creating Undo history.
- External MCP discovery is now **73 tools**, adding `creative.propose_direction`, `creative.get_direction`, `creative.revise_direction` and `creative.reopen_direction`. `motion.plan_opportunities` and raw-video scene creation fail closed before exact Style Lock authority and preserve Style Lock ID/content hash/proposal revision downstream.
- Fixed hard-style semantics so component defaults can be deterministically normalized to approved palette/font roles but cannot widen approved constraints. Creative artifacts now require exact `styleLockContentHash` and `creativeDirectionRevision` provenance. Workspace `.md` and bounded brand/brief/style/guideline `.txt` files are safely discoverable as confined Creative Brief evidence and cannot be consumed as transcripts.
- Deterministic real STDIO acceptance proves pre-approval planning refusal, revision-2 direction review, stale revision-1 refusal, trusted test-host approval into `stylelock_46622b0a92ffbfac`, exact opportunity-map provenance, and then the existing source-composited general Storyboard authoring battery with production revision unchanged. Three fresh STDIO processes separately prove pending-review and post-approval Style Lock reconnect continuity.
- Real Codex `0.144.1` independently proves the model-facing gate without manufacturing human visual approval: thread `01a05db2-1fcb-7222-a5eb-444f67385c1b`, run `run_00d140v0`, proposal `direction_00nwsubu` revision 2, review `review_01j0yrfk`, one review-board artifact, required planning refusal before/after revision, production revision still 0. Final marker: `SANVERSE_CODEX_DIRECTION_GATE_OK run_id=run_00d140v0 proposal_id=direction_00nwsubu revision=2 review_id=review_01j0yrfk artifact_count=1 plan_blocked=true production_revision=0`.
- Authoritative clean Windows single-fork repository regression exits 0: Web 1,239/1,239; API 411/411; Creative Direction 46/46; Creative Adapter 37/37; Edit Domain 491/491; Motion Library 202/202; Motion Graph 148/148; Render Contract 121/121; Motion MCP 30/30; Motion Agent Tools 30/30; Motion Storyboard 16/16; all remaining workspaces also pass. Production dependency audit is 0 vulnerabilities across 153 production dependencies. Evidence and transparent failure registry: `DOCS/evidence/2026-09-01-creative-direction-style-lock-v1/`.
- Broad engineering authorization was not converted into a claim that the owner visually approved one exact Creative Direction board. The separate REQ-020 meaningful-spoken-video/~10-scene/real approval/1× watch release tag gate remains open.
...
- Closed the Storyboard → Motion authority gap: the exact owner-approved Storyboard graph is now the Motion Forge structural baseline, with acceptance proving authored design semantics survive into Motion instead of reverting to the original component candidate.
- Added source-composited Storyboard chat review. Real KVS review frames use the exact canonical source tick and current graph/treatment; the source decoder remains bounded to its existing 640 px derived-preview envelope and the compositor scales that frame behind the graph.
- Registry discovery is **69 tools**. Deterministic real-project STDIO acceptance passes `$29 → £29`, shape → `ellipse`, structural duplicate + semantic-part redesign, exact KVS source mapping, semantic diffs, chat-attached review images and zero accepted-production mutation.
- Real Codex `0.144.1` acceptance passes after typed fail-closed self-correction: final marker `SANVERSE_CODEX_AUTHORING_OK run_id=run_01ehbqzl scene_id=creative_scene_01srm0io review_id=review_00nry3ss image_count=2`; Codex explicitly verified U+00A3 code point 163 plus `29`, `ellipse`, source-composite review and unchanged production revision.
- Authoritative Windows single-fork root tests exit 0; changed/high-risk surfaces include Web 1,239/1,239, API 411/411, Edit Domain 491/491, Motion Library 202/202, Motion Graph 148/148, Render Contract 120/120, Creative Adapter 35/35, Motion MCP 30/30, Agent Tools 30/30 and Storyboard 16/16. Root all-workspace production build exits 0. Final hygiene: `git diff --check` PASS; 0 `sites/**` changes; 0 raw-media, private-path or secret-like additions; production dependency audit 0 vulnerabilities at every severity. Failures and fixes are retained under `DOCS/evidence/2026-09-01-general-storyboard-authoring-v1/`.
- The separate REQ-020 human raw-video release tag remains withheld; engineering completion does not substitute for the unseen 1× human review gate.
...
- Added safe allowlisted local MP4 intake through the existing production project endpoint, analysis-only plain/SRT/VTT transcript context, project-backed Source Understanding, deterministic capability-aware opportunity maps, multiple isolated Creative scene sessions and exact Storyboard → Animatic → Motion approval gates. External JSON cannot manufacture `OwnerApprovalV1`; only opaque host-issued approval references resolve exact revision-bound approvals.
- Added immutable project-local SHA-256 Creative artifacts and one generic render-affecting `add-creative-scene` edit operation. Multiple approved scenes are staged/validated and accepted as **one** production ChangeSet against one live revision, so one Undo removes the whole batch and one Redo restores it. Portable archive/repository/history/render identity paths preserve the new operation/artifact authority.
- Integrated accepted Creative scenes into normal production preview and the existing export job. Preview and export both load the same immutable artifact into the same Motion Library module and canonical Motion runtime. Exact frame→tick materialization produces reproducible transparent PNG cache only; existing FFmpeg then composites those frames with the production source/audio pipeline.
- Real Edge/CDP parity across **3 scenes × 5 critical ticks** proves preview-surface pixels equal export-surface pixels byte-for-byte and direct/backward/random seek at the same tick is byte-identical. Final H.264 composition comparison minimum SSIM is **0.969474**, above the **0.96** gate.
- The real export proof found two Windows long-path defects and fixed both at ephemeral boundaries: Edge's disposable CDP profile moved to a short OS-temp directory; the disposable render workspace moved to a short random same-volume sibling under Sanverse's data-root `projects` directory. The latter prevented Windows/Node from misreporting an existing FFmpeg binary as `ENOENT` when the old working directory was too deep.
- Fresh isolated official Streamable HTTP MCP acceptance then passed from **0 projects** with **52 tools**: MP4 import → SRT source context → analysis → 3 opportunities → 3 scenes → 3 Storyboard + 3 Animatic + 3 Motion test-host approvals → one three-operation Creative ChangeSet at revision 1 → real export → one Undo → one Redo. Independent FFprobe reports H.264 **1280×720 @30fps**, AAC **48 kHz stereo**, exactly **12.000s**, **7,174,167 bytes**; production and downloaded SHA-256 both equal `a6657b0bd18e624d43631ecfc23a2c505b838a955a18082cb444f275b48fe3ee`.
- Official STDIO discovers the same **52 tools**, selects the production project, completes the prior V1.6 sandbox edit/review/discard workflow and leaves accepted project state byte-identical. Codex `0.144.1`, Claude Code `2.1.168` and OpenCode `1.18.8` endpoint/config verification is green with no model/provider call.
- The first default parallel root test run produced four web 5-second timeouts with no assertion mismatch. Each exact test passed alone in roughly 1.0–1.25s; the complete documented Windows single-fork web suite then passed **1,238/1,238**, and the final all-workspace single-fork matrix passed **2,990/2,990 across 25 workspaces**. Root all-workspace production build exits 0.
- Durable automated RC evidence is `DOCS/evidence/2026-08-30-external-mcp-raw-video-v1/`. The final immutable tag `sanverse-external-mcp-raw-video-v1` is intentionally withheld because the separate release contract still requires a meaningful spoken-video ten-opportunity/ten-scene benchmark, legitimate human Storyboard/timing/Motion review, representative multi-family parity evidence and an actual 1× watch of the complete final export. Test-host approvals and broad authorization to continue implementation are not recorded as unseen visual review.

## 2026-08-28 — Creative Engine V1.6 production editor integration sealed locally

- Added `@sanverse/creative-production-adapter` as the narrow production seam over existing edit-domain, Motion Graph, Motion Library, closed-loop workflow and internal-tool authorities. Source context is revision-bound and exact-tick. All 99 canonical Library entries are surfaced truthfully; only Kinetic Headline has a lossless V1.6 production adapter, while the other 98 remain explicitly Creative-preview-only rather than being flattened into a fake production representation.
- Integrated a persistent Creative Studio workspace into `apps/web` without a second editor/video/project/history/preview/export authority. Storyboard/KVS, Animatic, Motion Forge, Motion Review and real C3/C4/C5/C6 projections share one semantic node selection. A manual C5 preset uses canonical Motion Graph operations and resets stale approvals before the changed scene can be reapproved.
- Production apply crosses the existing server-authoritative change-set path as exactly one change set containing `add-title` + `set-visual-properties` with AI provenance and Creative lineage. App integration proves one Undo removes the complete Creative result, one Redo restores it and the accepted result survives reopen.
- Added a production-fenced view of the existing internal registry for UI/MCP. Mutating MCP tools require the live production revision and fail with `STALE_PRODUCTION_REVISION` before sandbox mutation. Isolated Creative apply/Undo tools are deliberately omitted so accepted-project history stays owned by the production editor/server.
- Proved exact production preview/export parity and all four production ratios 16:9, 9:16, 1:1 and 4:5. Failure recovery is fail-closed for missing source, unsupported source timing, stale project revision, stale MCP revision and failed apply.
- Real Edge verification found and fixed a 640×360 Kinetic Headline minimum-font bound, a Creative light-theme contrast bug caused by a nonexistent token, and a UX gap where export completion/download could be hidden inside a collapsed AI panel. Export status/download is now globally reachable from EditorShell while the detailed existing export panel remains intact.
- Final real Edge production audit used a temporary synthetic-real 640×360, 6.000s MP4 and proved C5 `snappy` edit → approval invalidation → explicit Storyboard/Animatic/Motion reapproval → atomic apply revision 1 → Undo revision 2 → Redo revision 3 → persisted reopen. Desktop 1440×900, tablet 1024×768 and mobile 390×844 have no horizontal overflow, exactly one video and reachable Creative controls. The real exported H.264/AAC MP4 is 640×360, 30fps, 6.000s, 858,823 bytes, SHA-256 `653afd857d0fc4d3441c9486dfac564af9d18e4503c80f45c6f1b96287ce12ef`; browser console errors 0, network failures 0, bad responses 0. Evidence: `DOCS/evidence/2026-08-28-creative-engine-v16-production/`.
- Final authoritative Windows single-fork matrix is **2,950/2,950 PASS across 25 workspaces**, including Web **1,238/1,238**, Creative production adapter **12/12**, API **403/403**, edit-domain **488/488**, Motion Graph **139/139**, Motion Library **202/202**, MCP **18/18** and render-contract **119/119**. Root all-workspace production build exits 0. `git diff --check` passes; `sites/**` diff 0; raw media/export additions 0; secret-like additions 0; no real parallel-authority matches. Release is sealed locally by immutable tag `sanverse-production-editor-creative-engine-v1.6`; no GitHub remote operation occurred.

## 2026-08-28 — Creative Engine V1.5 performance maturity + advanced bridges sealed locally

- Added C13 evidence-only performance recording plus `prepareMotionSceneEvaluatorV15`, which validates an immutable canonical scene once for hot preview loops while one-shot `evaluateScene` retains the original validation boundary. The canonical stress scene is 524 graph nodes with 2,524 animated properties and 24 animated masks over exactly 14,400,000 ticks / 10 seconds.
- Added deterministic direct/back/random seek, ten-track source-aware, camera/depth, long-project serialization/reference/resource cleanup and preview/export parity probes. The Library scale index searches 5,000 canonical catalog-shaped entries without creating a second registry. Atomic MotionPlan batching compiles dependent intents privately and commits one reversible canonical operation batch.
- Added explicit Expert Runtime LIGHT/MEDIUM/HEAVY/EXTREME host budgets with fail-closed `EXPERT_BUDGET_EXCEEDED`. Extended the existing internal T0/T1 registry and thin MCP adapter with V1.5 batching/performance/Three/AEP/MOGRT operations; MCP still owns no project, approval, graph or Undo state.
- Added truthful advanced foreign-source bridges: only the declared deterministic `sanverse.three-subset/v1` plane/circle/basic subset can become native Motion Graph state; arbitrary network/time/random/custom-shader Three/WebGL source rejects or requires flattening. Raw AEP/MOGRT needs trusted extraction; `sanverse.adobe-extract/v1` manifests classify supported/translated/flatten/unsupported features and never pretend universal compatibility.
- Fresh real Edge `/performance-review` ran true 1× for **10,179 ms**, reached **14,400,000/14,400,000** ticks, captured **102** screencast frames across **190** full prepared evaluations, passed direct/back/random, tracking, camera, graph preview/export and actual browser text-pixel parity, reported prepared p95 **10.0 ms** / max **24.8 ms**, leaked **0** resources and had zero console/network failures.
- Final focused V1.5 gate is **493/493** across Motion Graph, Agent Tools, MCP, Expert Runtime, External Bridge, Performance, Library and Motion Lab. Authoritative Windows single-fork repository matrix is **2,930/2,930 PASS**, including protected Web **1,231/1,231**; root all-workspace build exits 0. Final isolation has `apps/web/**` diff 0 and `sites/**` diff 0, no raw source-media additions, no secret-like additions and no parallel-authority matches. Release is sealed locally by immutable tag `sanverse-creative-engine-v1.5`; V1.6 may start only from that exact commit in a dedicated local integration worktree.

## 2026-08-27 — Creative Engine V1.4 Expert Motion release candidate green

- Added one bounded canonical Motion Graph `expert` leaf and `@sanverse/motion-expert-runtime` for fixed procedural `orbital-rings`, analytically reconstructable `radial-burst` particles and bounded `plasma-field` shader plans. Expert outputs are pure functions of exact 1,440,000-based tick + typed parameters/assets + explicit seed; there is no retained particle simulation, wall-clock time, ExpertGraph, ParticleGraph or ShaderTimeline.
- Added fail-closed expert resource validation and fixed complexity ceilings. Arbitrary JavaScript, GLSL/WGSL, URLs, callbacks, filesystem/network access, autonomous clocks and unknown/over-budget programs are refused before graph/runtime evaluation.
- Extended the existing rights/provenance-gated external bridge with bounded declarative procedural/shader subsets that materialize to the canonical expert node. Added three vetted Expert Recipes, internal registry operations first, and thin MCP exposure second; MCP owns no expert/project/approval/Undo state.
- Added native `MotionExpertNodeSurface`, development-only `/expert-motion-review`, and a real Edge audit. Final Edge proof ran true **1×** for **5075 ms**, captured **141** frames, reached exactly **7,200,000/7,200,000** ticks, proved direct/backward/random seek equality + resource QA, projected the same **3** expert IDs through C3 and C6, rendered **8** rings + **72** analytic particles + shader seed **303**, and had zero release console/network failures. Entrance/middle/final frames were manually inspected and were visually clean.
- Focused gates: Motion Graph **139/139**, Expert Runtime **9/9**, External Bridge **18/18**, Agent Tools **25/25**, MCP **16/16**, Native Runtime **8/8**, Motion Lab **66/66**. The first heavily loaded repository run produced two unchanged protected-Web 5-second timeouts with no assertion failures; the exact tests passed alone in **2.152 s** and **1.551 s**. The final complete repository rerun passed **2,909/2,909**, including protected Web **1,231/1,231**, exit 0. Root all-workspace build **PASS / exit 0**.
- Release is sealed locally at implementation/evidence commit `b6a833f3f8254c0a2cf5c1b4b54ebc9cb12ffaf7` with immutable tag `sanverse-creative-engine-v1.4` resolving to that exact SHA. The first post-tag default parallel Windows run produced three unchanged protected-Web 5-second timeouts with no assertion failures; those exact cases passed alone in **2.324 s**, **2.209 s** and **4.615 s**. The repository-documented Windows single-fork full matrix then passed **2,909/2,909** including protected Web **1,231/1,231**, exit 0, and the post-tag root all-workspace build exited 0. Final isolation: V1.3→V1.4 `apps/web/**` diff 0, `sites/**` diff 0, no raw user/source media, no GitHub remote/Actions work, and no C13/universal Three-WebGL/AEP-MOGRT/production-editor work started.
...
## 2026-08-31 — Chat-first review + resumable Creative Run V1

The raw-video external workflow no longer loses its Creative batch when Codex/OpenCode disconnect. Added a bounded project-local `CreativeRunV1` document that persists source understanding, opportunity selection, serialized multi-scene Closed-Loop state and exact review records. A new session rehydrates the existing workflow engines from that document and refuses stale project revision, scene identity or corrupted state instead of guessing.

Review is now owner-facing instead of plumbing-facing. Canonical Storyboard/Animatic/Motion frames are materialized through the existing Creative render path, written as bounded review artifacts, SHA-256 bound to the exact review, rehashed before presentation, and attached directly as MCP image content. The owner no longer has to supply an internal approval reference. A local STDIO client without elicitation can fall back to an automatically opened loopback browser confirmation page with one-time URL/nonce protection and exact artifact-hash checking.

The security boundary remains host-only. `creative.decide_review` can describe an intended decision but ordinary tool JSON cannot create `hostReviewDecision`. The v2 multi-round-trip path uses signed/TTL-bound request state, revalidates exact run/review/evidence/subject revision on reentry, and only then lets the trusted host approval issuer create the existing opaque approval. Review-purpose artifacts are explicitly non-production; the existing production artifact validator still requires Motion approval.

Localized repair now behaves like a real multi-scene workflow: revision resets only the targeted scene's downstream lineage and rejection removes only that scene. Opportunity planning changed from an exact quota to **up to N useful moments** with independent candidate validation, conservative small-overlap trimming and per-candidate rejection, so one bad custom moment no longer destroys valid tailored siblings or forces generic filler.

Real STDIO acceptance discovered **62 tools**, returned three Storyboard frames, completed one trusted owner-confirmation round, returned only the three new Animatic frames, disconnected, then resumed the same pending Animatic review with the same three frames. Real Codex and real OpenCode independently resumed/read that persisted review without mutation. Focused suites are Render Contract **120/120**, Creative Adapter **34/34**, Motion MCP **30/30**, Motion Agent Tools **30/30**, run store **2/2**, browser fallback **2/2**. The full all-workspace repository test command exits 0, the root all-workspace production build exits 0, and final production dependency audit is zero at all severities.

Engineering evidence: `DOCS/evidence/2026-08-31-creative-run-chat-review-v1/`. The broader raw-video final release tag remains withheld; this slice does not replace its separate legitimate owner multi-stage/final-video review gates.
