...
- Release-candidate hygiene: `git diff --check` PASS; actual `apps/web/**` diff 0; no raw user/source media; `sites/` remains separately versioned/excluded; no GitHub remote/Actions work. Local implementation/evidence commit + immutable `sanverse-creative-engine-v1.4` tag are the next atomic gate, followed by the required post-tag V1.0→V1.4 regression/build/isolation audit.
