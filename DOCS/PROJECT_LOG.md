...
- Release hygiene: `git diff --check` PASS; `apps/web/**` diff 0; no raw source media; separately versioned `sites/` remains excluded; no GitHub remote/PR/Actions operation. Continuous status: `DOCS/motion/creative-engine-v1.2-v1.4/STATUS.md`. Local V1.2 commit/tag is the next gate before any V1.3 work.
