...
- MOTION-C3 — **implementation + release-candidate verification complete**. Professional Layer Hierarchy now projects directly from Motion Graph, with a constant render-enable switch separate from animated visibility, persistent lock metadata, ancestor-effective enable/lock, canonical single/multi selection, graph-derived search/badges, Layers↔Preview selection synchronization, conservative node Inspector, C1-routed rename/reorder/local-transform reparent/duplicate/delete/group/ungroup/effects/masks, bounded Motion Lab Undo/Redo, and browser-measured selection overlays. All 69 public components project at all four ratios. Evidence: `DOCS/motion/evidence/MOTION-C3.md`; architecture: `DOCS/motion/LAYER_ARCHITECTURE.md`.
- C3 release-candidate gate — **304/304 Motion tests** and **7/7 Motion workspace builds** passed. Final source scans show zero `apps/web` changes/imports, zero Plan-B logic, zero prohibited animation runtimes and no C4 implementation. C3 must be committed/pushed/tagged as `motion-compositor-c3` before A19 source work begins.
- MOTION-A19 — **next after the C3 Git checkpoint**. It must re-read the 69-component coverage matrix and add only 8–10 genuinely missing hierarchy-heavy explainer scenarios.
