...
- **Active engineering line:** `external-mcp-raw-video-v1`; verify the newest HEAD with Git. The latest engineering slice is **REQ-026 — Canonical Motion Graph Supplemental Renderer V1**.
- REQ-026 closes the reported missing-pixel blocker: registered component-native visuals remain bespoke, while newly authored enabled/visible canonical graph nodes reachable from `rootNodeId` now receive a shared supplemental pixel path through the same `MotionComponentHost` used by Storyboard review, preview and export. Real source-composited Edge proof shows native Floating Value Cloud visuals plus authored magenta/cyan shapes and custom text together; preview/export screenshots are byte-identical and production revision is unchanged. V1 supplemental nodes composite above the bespoke component surface rather than claiming arbitrary interleaving inside every component DOM subtree.
- Raw-video flow remains Source Understanding → **Creative Direction owner gate** → opportunity planning → Storyboard → Animatic → Motion. Brand Context is evidence only. A persisted/revisable Creative Direction proposal is not authority; only exact trusted-host approval of one review revision/evidence hash creates the content-derived Approved Style Lock. Reopening direction invalidates downstream active authority without mutating accepted production state.
...
- The authoritative clean single-fork repository regression exits `0`: Web **1,239/1,239**, API **411/411**, Creative Direction **46/46**, Creative Production Adapter **37/37**, Edit Domain **491/491**, Motion Library **202/202**, Motion Graph **148/148**, Render Contract **121/121**, Motion MCP **30/30**, Motion Agent Tools **30/30**, Motion Storyboard **16/16**, and the changed Motion Native Runtime **14/14**, with all other workspaces green. Root all-workspace production build exits `0`; production dependency audit is 0 vulnerabilities at every severity. Renderer evidence: `DOCS/evidence/2026-09-01-motion-graph-renderer-v1/`; Creative Direction evidence remains `DOCS/evidence/2026-09-01-creative-direction-style-lock-v1/`.
