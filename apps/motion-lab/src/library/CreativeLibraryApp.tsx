...
function LibraryHome({ search, navigate, reviewByComponent, reviewLoading }: Readonly<{ search: string; navigate: (href: string) => void; reviewByComponent: Readonly<Record<string, MotionQualityReviewV1>>; reviewLoading: boolean }>) {
  ...
  const newest = reviewedCatalog.filter((entry) => entry.introducedInMilestone === 'A21')
  return <main className="creative-library"><LibraryHeader count={reviewedCatalog.length} navigate={navigate} />
    ...
    {!options.query && !options.category && !options.collectionId && newest.length > 0 ? <section className="creative-library__recent"><div><span>RECENTLY ADDED</span><strong>A21 · {newest.length} components</strong></div><button onClick={() => update('collection', 'recently-added')}>View A21 →</button></section> : null}
